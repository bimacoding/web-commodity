...
class Menu extends CI_Controller {
	...
	function index()
	{
		...
		$this->template->load('backend/template','backend/mod_master_menu/view',$data);
	}
	...
	function tambah_menu()
	{
		if (isset($_POST['submit'])) {
			...
			$this->template->load('backend/template','backend/mod_master_menu/add',$data);
		}
	...
	function ubah_menu()
	{
		...
		if (isset($_POST['submit'])) {
			...
			$data['menus'] = $this->model_app->view_where_order('t_menu',array('id_parent'=>0,'position'=>'Side','aktif'=>'Ya'),'id_menu','DESC');
			$this->template->load('backend/template','backend/mod_master_menu/edit',$data);
		}
