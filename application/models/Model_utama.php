...
class Model_utama extends CI_model{
    ...
    public function view_join_one_not($table1,$table2,$field,$wh,$where,$order,$ordering,$baris,$dari){
        $this->db->select('*');
        $this->db->from($table1);
        $this->db->join($table2, $table1.'.'.$field.'='.$table2.'.'.$field);
        $this->db->where_not_in($wh,$where);
        $this->db->order_by($order,$ordering);
        $this->db->limit($dari, $baris);
        return $this->db->get();
    }
    ...
    function cari_post($kata){
        $pisah_kata = explode(" ",$kata);
        $jml_katakan = (integer)count($pisah_kata);
        $jml_kata = $jml_katakan-1;
        $cari = "SELECT * FROM t_post a join t_kategori c on a.id_kategori=c.id_kategori
                       WHERE a.publish='Y' AND";
            for ($i=0; $i<=$jml_kata; $i++){
              $cari .= " a.judul_post LIKE '%".$pisah_kata[$i]."%'";
                if ($i < $jml_kata ){
                    $cari .= " OR "; 
                } 
            }
        $cari .= " ORDER BY a.id_post DESC LIMIT 10";
        return $this->db->query($cari);
    }
